...
def bndplot(datafile,fermi,symmetryfile,subplot,label):
  ...
  subplot.plot([min(x),max(x)],[Fermi,Fermi],color='red',label="Fermi Level")
  subplot.xticks(np.linspace(axis[0],axis[1],11),label)
  subplot.ylim(0,40)
  subplot.xlim([axis[0],axis[1]])
  #subplot.text((axis[1]-axis[0])/2.0,axis[3]+0.2,label,va='center',ha='center',fontsize=20)
  subplot.xlabel("k points",fontsize=18)
  subplot.ylabel("Energy Level(eV)",fontsize=18)
  subplot.title("Plot of Band Structure for Li2O2 crystal",fontsize=30)
  subplot.legend()
  subplot.show()
